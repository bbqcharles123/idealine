...
// 반환값: { question, answerExamples } — answerExamples는 정제를 거친 0~3개의 문장 배열
//
...
const QUESTION_SCHEMA = {
  ...
  schema: {
    ...
    // strict 모드는 properties의 모든 키가 required에 있어야 한다 — 빼면 API가 400으로 거부한다.
    // 그래서 answerExamples는 "키가 반드시 오는" 값이 되지만, 배열의 내용은 보장되지 않는다.
    // (minItems/maxItems를 strict가 지원하지 않는 사정은 normalizeExamples 위 주석과 같다)
    // → 개수·길이는 프롬프트로 요구하고, 모자란 경우는 아래 정제 단계에서 받아낸다.
    required: ['question', 'answerExamples'],
    properties: {
      // ── properties 순서가 동작에 영향을 준다 ──
      // JSON은 앞에서부터 생성되므로, question이 먼저 있어야 모델이 질문을 확정한 뒤
      // 그 질문을 보고 후보를 만든다. 순서를 뒤집으면 질문 없이 후보부터 쓰게 되어
      // [답변 후보 규칙]의 "위에서 작성한 질문에 대한 답"이 성립하지 않는다.
      question: { type: 'string', description: '사용자가 해당 도구를 적용해 아이디어를 발전시키도록 유도하는 질문 1개' },
      answerExamples: {
        type: 'array',
        description: '사용자가 question에 답하지 못할 때 골라 쓸 수 있는 답변 후보 3개. 각 항목은 사용자가 직접 쓴 것처럼 1인칭으로 서술한 40~70자 한 문장',
        items: { type: 'string' },
      },
    },
  },
...
export async function generateQuestion(cardDescription, toolName, toolType) {
  ...
  //
  // [사용 맥락] 블록을 두는 이유
  // 예시 생성(toolExamplesPrompt.js)·본문 생성·직접작성 프롬프트는 산출물이 화면에서 어떻게
  // 쓰이는지를 블록으로 알려주는데, 질문 생성에만 그 블록이 없었다.
  // 용도를 적지 않으면 모델은 질문을 '거기서 끝나는 산출물'로 여긴다
  // (예시 생성에서 관찰된 것과 같은 계열의 문제 — 그쪽은 선택지를 완성된 제안서로 썼다).
  // 세 문장이 각각 다른 것을 겨냥한다.
  //   ① 사용자가 읽고 답한다        → 최종 결과물이 아니라 답할 대상이라는 것
  //                                  ('답하기 쉽도록 열린 질문' 규칙의 근거)
  //   ② 답변이 다음 카드의 재료다   → 한 단어로 답하고 끝날 질문을 막는 근거
  //                                  (그 다음 호출이 generateDerivedContent다)
  //   ③ 사용자는 도구 정의를 모른다 → 도구 용어를 그대로 쓴 질문은 답할 수 없다는 조건
  //                                  (본문 생성의 [하이라이트 규칙]도 같은 전제를 쓴다)
  //
  // [작성 규칙] 헤더를 씌우고 '위 도구'를 '[도구 설명]에 적힌'으로 바꾼 이유
  // 규칙 4줄이 라벨 없이 떠 있어서, 마지막으로 열린 [선택 맥락] 블록의 하위 항목으로 읽혔다.
  // (다른 프롬프트는 전부 [작성 규칙] 같은 헤더 아래에 규칙을 둔다)
  // 문제는 첫 줄의 '위 도구'였다 — 가리키려는 것은 [도구 설명]의 정의인데,
  // 그 줄 바로 위에 있는 것은 [선택 맥락]의 방향성 문장이다.
  // 방향성은 한 묶음에 속한 도구들의 공통점 서술이라(예: 없애거나 바꾸거나 나누는),
  // 그쪽을 사고 방향으로 삼으면 '제거'가 아닌 '대체'에 맞는 질문도 규칙을 지킨 것이 된다.
  // 같은 성분의 사고가 두 번 기록돼 있다 — 방향성을 [도구 설명] 안에 넣었을 때 도구 정의처럼
  // 읽힌 일(위 contextSection 주석), 예시 생성에서 reasoning이 변별을 무너뜨린 일
  // (toolExamplesPrompt.js). 블록 분리로 전자는 고쳤지만 참조 쪽에 같은 문제가 남아 있었다.
  // → 위치('위')가 아니라 이름('[도구 설명]')으로 가리키게 해서
  //   블록 순서나 [선택 맥락]의 유무(조건부 섹션이다)와 무관해지도록 했다.
  //
  ...
  //
  // [후보의 다양성 규칙을 '요소'에서 '답'으로 바꾼 이유]
  // 처음에는 "세 후보는 [아이디어]의 서로 다른 요소를 건드려야 합니다"와
  // "달라지는 것은 '무엇에 적용했는가'입니다"로 축을 요소에 못 박아 두었다.
  // 실제 호출에서 세 후보가 전부 같은 요소로 시작하는 경우가 나왔다.
  //   "단계별 동영상 강의를 5개에서 3개로 줄여서, ..."
  //   "단계별 동영상 강의를 간소화하여, ..."
  //   "단계별 동영상 강의를 줄여서, ..."
  //
  // 모델이 규칙을 어긴 것이 아니라 규칙끼리 충돌한 것이다.
  // [작성 규칙]은 질문에 요소를 하나 골라 넣으라고 요구하고([아이디어]에 적힌 것 중 하나),
  // [답변 후보 규칙] 1번은 후보가 그 질문에 대한 답이어야 한다고 요구한다.
  // 질문이 "단계별 동영상 강의의 수를 줄이면"처럼 요소를 못 박은 순간,
  // '서로 다른 요소'는 '질문에 대한 답'과 동시에 만족시킬 수 없다.
  // (질문이 열려 있던 다른 호출에서는 세 후보가 실제로 갈렸다 — 질문의 폭에 따라 깨지는 구조였다)
  //
  // → 축을 좁게 지정하지 않는다. 대신 두 가지만 남겼다.
  //   ① 아이디어를 먼저 읽고 적용 지점을 그 안에서 찾을 것
  //      (질문의 요구를 어디에 어떻게 적용할지는 아이디어를 봐야 알 수 있다)
  //   ② 세 후보가 서로 바꿔 놓아도 똑같이 읽히면 실패
  // 요소가 고정된 질문에서는 정도·대상·대체물에서 갈리면 ②를 통과하고,
  // 열린 질문에서는 요소에서 갈려도 통과한다. 질문의 폭과 무관하게 성립한다.
  //
  // 도구 일관성은 따로 요구하지 않는다 — 질문 자체가 도구의 조작을 담고 있으므로
  // "질문이 요구하는 조작"이라고 가리키면 그쪽으로 따라온다.
  //
  // [답변 후보의 길이에 하한을 둔 이유]
  // 처음에는 상한("70자 이내")만 두었는데, 실제 호출에서 24~30자짜리 한 줄 문장만 나왔다.
  //   "매주 새로운 메뉴 대신 고정된 메뉴로 단순화했다." (24자)
  // 문제는 모달 높이가 아니라 재료의 빈약함이다. 이 답변은 그대로 answer가 되어
  // generateDerivedContent의 입력이 되고, highlightPhrases도 이 문장 안에서만 뽑힌다.
  // 무엇을 바꿨는지만 있고 그래서 무엇이 달라지는지가 없으면 파생 카드가 얕아진다.
  //
  // 그래서 숫자만 바꾸지 않고 "두 부분을 담으라"를 함께 적었다.
  // 모델은 글자 수를 세지 못하므로 40~70자라는 범위만으로는 잘 지켜지지 않는다.
  // 담을 내용을 지정하면 길이는 그 결과로 따라온다 — 40~70자는 그 확인용 눈금이다.
  // (한 줄 33~35자 기준이라 70자는 선택지 2줄 = 시안의 745px 높이와 같다)
  //
...
[사용 맥락]
작성한 질문은 모달에서 사용자에게 제시되고, 사용자는 그 질문에 대한 답을 직접 씁니다.
사용자가 쓴 답변은 AI에 전달되어 새로운 아이디어 카드가 만들어지는 재료가 됩니다.
사용자는 이 도구의 정의를 알지 못합니다. 질문에 답을 고민하는 과정에서 자신도 모르는 사이에 그 도구를 사용하게 됩니다.
질문과 함께 만드는 답변 후보 3개는, 사용자가 답을 떠올리지 못할 때 골라 쓸 수 있도록 같은 화면에 제시됩니다.
사용자는 고른 후보를 그대로 제출하거나 고쳐 씁니다. 후보는 완성된 모범답이 아니라 사용자가 이어받을 출발점입니다.

[도구 설명]
${toolName}: ${toolDef}${contextSection}

[작성 규칙]
- 질문은 [도구 설명]에 적힌 조작의 사고 방향에 정확히 맞아야 합니다.
...
[답변 후보 규칙]
- 후보 3개를 만듭니다. 각 후보는 위에서 작성한 질문에 대한 답이며, 사용자가 직접 쓴 것처럼 1인칭으로 서술합니다. 제안하거나 설명하는 문장이 아닙니다.
- 답을 쓰기 전에 [아이디어]를 읽고, 질문이 요구하는 조작을 그 안의 무엇에 어떻게 적용할지 찾으세요. 
- 질문은 [아이디어]에 이미 적힌 요소를 지목하지만, 후보는 거기에 아이디어에 없던 것을 더합니다. 그것이 발전입니다.
- 각 후보는 40~70자의 한국어 한 문장입니다. 무엇을 바꿨는지만 적고 끝내지 말고, 그래서 무엇이 달라지는지까지 한 문장 안에 담으세요.`

  // user: 이번 호출에만 해당하는 데이터(부모 카드 본문 + 적용할 도구명)와 [요청].
  // [요청] 블록의 형식은 다섯 호출이 같다 — 데이터 블록의 역할을 한 줄씩 적고,
  // 마지막에 무엇을 만드는지 한 문장. 규칙(길이·금지·검사)은 system의 [작성 규칙]에만 둔다.
  const user = `[아이디어]
${cardDescription}
...
[요청]
[아이디어]는 사용자가 지금 발전시키려는 카드의 본문이며, 도구가 적용될 대상입니다.
...
[적용할 도구]를 [아이디어]에 적용해 보도록 유도하는 질문 1개를 작성합니다.`
  ...
  //
  // [답변 후보를 함께 만들게 된 뒤에도 0.4를 유지하는 이유]
  // 원래 이 자리에는 "산출물이 질문 1개라서 사용자가 고를 대안이 없다"는 근거가 하나 더 있었는데,
  // 후보 3개가 생기면서 그 문장은 사실이 아니게 되어 지웠다.
  // 그래서 한 호출 안에 요구가 상반된 두 산출물이 놓인다 —
  // 질문은 입력에 충실해야 하고(낮은 온도), 후보 3개는 서로 달라야 한다(높은 온도).
  // 온도를 올리지 않는 쪽을 택한 이유는 두 가지다.
  //   ① 온도는 '무엇이' 달라질지 지정하지 못한다. 올리면 후보의 다양성과 함께
  //      질문의 "실제로 적혀 있는 요소를 그 표현 그대로" 규칙까지 같이 흔들린다.
  //   ② 다양성은 프롬프트로 검사할 수 있다. [답변 후보 규칙]의 바꿔치기 검사는
  //      '무엇이 다르면 통과인지'를 모델이 스스로 판정할 수 있는 형태로 적어둔 것이라,
  //      전체를 무작위로 흔드는 온도보다 겨냥이 정확하다.
  // 질문이 망가지면 후보도 같이 망가진다(후보는 그 질문의 답이므로) — 질문 쪽을 지키는 것이 우선이다.
  ...
  // 후보 정제: 빈 문자열을 걷어내고 3개로 자른다.
  //
  // normalizeExamples(2단계 선택지)와 달리 개수가 모자라도 throw하지 않는다.
  // 2단계 선택지는 그것이 없으면 도구를 고를 수 없어 진행 자체가 불가능하지만,
  // 답변 후보는 없어도 사용자가 직접 답을 쓰면 그만이다 — 보조 수단이지 진행 조건이 아니다.
  // 여기서 throw하면 후보를 못 만든 것 때문에 멀쩡한 질문까지 버려지고,
  // 답을 쓸 수 있었던 사용자까지 오류 화면에 막힌다.
  // → 모자란 경우는 모달이 '예시 답변' 버튼을 비활성으로 두는 것으로 처리한다(2개 미만 기준).
  //
  // 모달 두 곳(확장·변형)이 같은 정제를 반복하지 않도록 ai 레이어에 둔다.
  const answerExamples = (res.answerExamples ?? [])
  ...
  logTransform(
    'tool_question',
    `답변 후보 ${answerExamples.length}개 확보 (2개 미만이면 모달이 '예시 답변' 버튼을 비활성으로 둔다)`,
    { 후보: answerExamples },
  )

  return { question: res.question, answerExamples }
}
...
async function generateDerivedContent(parentDescription, topic, question, answer, toolName, toolType, signal) {
  ...
  // system 구성:
  //   [도구 설명](toolPromptDesc) → [주제 경계](topicScope) → [사용 맥락](이 호출 전용)
  //   → [작성 규칙]: ▸재료 반영 규칙(이 호출 전용) + ▸아이디어 작성 규칙·▸실현 가능성 기준(ideaRules)
  //     + ▸하이라이트 규칙(이 호출 전용)
  //
  // 규칙 세 덩어리를 [작성 규칙] 한 블록으로 묶은 이유
  // 출처는 셋이 다르지만(공용 모듈 둘 + 이 호출 전용 하나) 프롬프트에서 하는 일은 같다 —
  // 모델이 지켜야 할 조건이다. 예전에는 셋이 각자 대괄호 블록이라 규칙 구역의 경계가 없었고,
  // 씨드·파생만 [작성 규칙]이라는 이름을 쓰지 않는 상태였다(다른 세 호출은 전부 쓴다).
  // 하위는 ▸ 소제목으로 낮춘다 — 대괄호로 두면 상위 [작성 규칙]과 형제로 읽힌다(ideaRules.js 주석).
  //
  // [사용 맥락]은 씨드 프롬프트에도 같은 이름의 블록이 있지만 내용이 달라 공유하지 않는다.
  // 씨드는 "발산의 출발점"이고, 파생은 완결된 아이디어이면서 동시에 다음 확장·변형의 부모다.
  //
  // [재료 반영 규칙을 [작성 규칙]으로 되돌린 경위]
  // 이 세 줄(부모를 다시 쓰지 않는다·답변을 옮겨 적지 않는다·사례를 동작 규칙으로 쓴다)은
  // 원래 buildIdeaWritingRule() 뒤에 한 줄로 붙어 있었는데, 공용 규칙의 일부인지 이 호출만의
  // 조건인지 프롬프트만 봐서는 갈리지 않아 [역할과 목적]으로 옮겼었다.
  // 그런데 그러면 맥락을 적는 블록에 지시문이 들어가 블록의 성격이 무너진다
  // (이름이 '목적'이라 규칙을 불러들인 셈이다 — 다섯 호출을 [사용 맥락]으로 통일한 이유).
  //   → [작성 규칙] 아래 ▸ 재료 반영 규칙이라는 자기 소제목을 준다.
  //     공용 규칙(▸아이디어 작성 규칙)과 나란히 놓이면서도 출처 구분이 남는다.
  //
...
[요청]
[부모 아이디어]는 사용자가 발전시키려는 카드의 본문, [질문]은 그 카드에 대해 제시된 질문, [사용자 답변]은 사용자가 그 질문에 직접 쓴 답입니다.
[사용자 답변]을 재료로 [부모 아이디어]를 발전시킨 파생 아이디어 카드 1개를 작성합니다.`
...
async function generateWriteContent(title, description, signal) {
  ...
  // 역할 선언 두 번째 줄은 원래 [상황] 마지막 문장이었다.
  // 다른 네 호출은 전부 역할 선언에서 "이번에 무엇을 만드는지"를 밝히는데 이 호출만 그게
  // 맥락 블록 안에 들어가 있었다 — 맥락과 지시가 한 블록에 섞이는 같은 문제다.
  //
  // [사용 맥락]에 선택지를 셋 다 적는 이유
  // 툴바 버튼은 확장하기·변형하기·직접작성 셋인데(Toolbar.jsx) 추천이 다루는 것은 앞의 둘뿐이다
  // (writeRec 스키마가 expand|transform 두 값만 받는다).
  // 셋 중 둘이라고 적지 않으면 모델이 사용자에게 남은 선택지가 둘뿐이라고 전제한다.
  // 화면 컴포넌트 이름(도구 레이어·사이드패널)은 적지 않는다 — 다른 [사용 맥락] 블록도
  // 표시 위치가 아니라 "사용자가 그것으로 무엇을 하는지"만 적는다.
  const system = `당신은 아이디어 발산 도구의 AI 어시스턴트입니다.
사용자가 도구의 도움 없이 직접 작성한 아이디어를 읽고, 그 내용만을 근거로 다음 단계에 '${TOOL_TYPE_LABEL.expand}'와 '${TOOL_TYPE_LABEL.transform}' 중 어느 쪽이 더 적합한지 판단합니다.

[사용 맥락]
사용자가 직접 작성한 이 아이디어는 캔버스에 카드로 추가되고, 사용자가 이어서 다음 아이디어를 만들 때 그 부모 카드가 됩니다.
그때 사용자가 고를 수 있는 것은 '${TOOL_TYPE_LABEL.expand}', '${TOOL_TYPE_LABEL.transform}', '직접작성' 세 가지이고, 당신의 추천은 그중 앞의 두 가지에 대한 것입니다.
작성한 추천 도구와 추천 이유, 기대효과는 그 선택을 앞두고 사용자에게 제시됩니다.
따라서 이 응답은 사용자가 따라야 할 지시가 아니라, 다음 단계를 고르는 데 쓰는 판단 재료입니다.
...
[요청]
[아이디어 제목]과 [아이디어 설명]은 사용자가 도구 없이 직접 작성한 아이디어입니다.
이 아이디어를 근거로 다음 단계에 적합한 도구를 골라 추천 도구·기대효과·추천 이유를 작성합니다.`
