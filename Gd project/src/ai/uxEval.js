...
${UX_RULE}`
  ...
  // user: 평가 대상 카드의 본문(제목 + 설명)과 [요청].
  // [요청] 블록의 형식은 다른 호출(seedCard.js·deriveCard.js)과 같다 — 데이터 블록의 역할을
  // 한 줄씩 적고, 마지막에 무엇을 만드는지 한 문장. 평가 규칙은 system의 UX_RULE에만 둔다.
  // 씨드·파생·직접작성 세 경로에서 모두 불리므로 본문의 작성 주체(AI/사용자)는 특정하지 않는다.
  const user = `[아이디어 제목]
${title}
...
[요청]
[아이디어 제목]과 [아이디어 설명]은 평가 대상 아이디어 카드의 본문입니다.
이 아이디어를 [평가요소] 7개 기준으로 평가해 평가요소별 평가·영역별 평가·종합요약을 작성합니다.`
